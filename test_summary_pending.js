...
async function test() {
  ...
  const getSummary = (patientId, start, end) => {
    const periodFats = (faturas || []).filter(
      (f) => f.paciente_id === patientId && 
             f.competencia >= start && 
             f.competencia <= end &&
    ...
    periodFats.forEach((f) => {
      if (f.especialidade === "Apoio") {
        ...
        if (items.length === 0) {
          ...
          summaryLines.push(`- ${spec}: ${profNome}`);
        } else {
          const groups = {};
          items.forEach((item) => {
  ...
  // Test Pedro Rafael in June 2026
  const pedro = pacientes.find(p => p.nome.includes("Pedro Rafael"));
  console.log("Pedro Rafael summary 2026-06:");
  console.log(getSummary(pedro.id, "2026-06-01", "2026-06-30"));

  // Test Théo Felipe in June 2026
  const theo = pacientes.find(p => p.nome.includes("Théo Felipe"));
  console.log("Théo Felipe summary 2026-06:");
  console.log(getSummary(theo.id, "2026-06-01", "2026-06-30"));
}
